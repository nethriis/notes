  ...
  <Layout>
    ...
    <Sidebar class="w-60 space-y-6 bg-white z-20">
      ...
      <div class="px-4">
        <button
          @click="() => newNote = true"
          class="w-full flex items-center justify-center py-1.5 bg-zinc-100 rounded-md text-sm font-medium"
        >
    ...
    <Sidebar class="w-72 mt-8 space-y-4 bg-zinc-100">
      ...
      <Suspense>
        <template #default>
          <FolderNoteList :newNote="newNote" @noteCreated="() => newNote = false" />
        </template>
        <template #fallback>
